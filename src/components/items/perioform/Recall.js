...
import {RecallForm} from '../../../redux/actions/ac.perio'
...
export class Recall extends Component {
    ...
    constructor() {
        ...
        this.state = {
            ...
            date : '',status:false,attempt:'',sel1:'Mild',sel2:'Gingivitis',validate:false,loading:false,month:''
        }
    }
    ...
    handleOnSend=(finished)=>{   
        ...
        let payload 
            if(finished){
                ...
                "finish":true,"date" : this.state.date,"HN":sessionStorage.getItem('HN'),"month":this.state.month,"doc_id":sessionStorage.getItem('student_id'),
                ...
                for (let i = 0; i < data.length; i++) {
                   ...
                   document.getElementById('recallcel0').disabled = true
                   document.getElementById('recallcel1').disabled = true
                   document.getElementById('recallcel2').disabled = true
                   document.getElementById('recallcel3').disabled = true
            }else{
                payload = {"answer" : data , "sel1":this.state.sel1 ,"sel2":this.state.sel2 ,"attempt" : Number(this.state.attempt),
                "date":this.state.date,"finish":false,"HN":sessionStorage.getItem('HN'),"month":this.state.month,"doc_id":sessionStorage.getItem('student_id'),
                "interval":this.props.interval,"kn":this.props.kn}
                setTimeout(() => {
                    ...
                    <Card>
                        <table border="1">
                            <thead className="th"><tr>
                                <td >Recall</td>
                                <td>
                                    <input id="recallsel0" onChange={(e)=>this.setState({attempt : e.target.value})} type='number'></input>
                                </td>
                                <td>
                                        <select class="form-control" id="recallsel1" name="sellist1" value={this.state.sel1} onChange={(e) => this.setState({sel1:e.target.value})}>
                                            <option value='Mild'>Mild</option>
                                            <option value='Moderate'>Moderate</option>
                                            <option value='Severe'>Severe</option>
                                        </select>
                                    </td>
                                    <td><select class="form-control" id="recallsel2" name="sellist2" value={this.state.sel2} onChange={(e) => this.setState({sel2:e.target.value})}>
                                ...
                                <td>Month
                                        <input type='number' value={this.state.month} id='recallsel3' onChange={(e)=>this.setState({month : e.target.value})}></input>
                                </td>
                                <td>
                                    <InputGroup >
                                        <InputGroup.Prepend>
